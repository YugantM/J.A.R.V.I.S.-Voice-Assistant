...
from vu_meter import audio_int
...
class Terrain(object):
    def __init__(self):
        ...
        verts = np.array([
            [
                x, y, 1 * self.tmp.noise2d(x=n / 5, y=m / 5)
    ...
    def update(self):
        ...
        if (audint != 0):
            verts = np.array([
                                 [
                                     x, y, audint * self.tmp.noise2d(x=n / 5 + self.offset, y=m / 5 + self.offset) / 2
                                 ] for n, x in enumerate(self.xpoints) for m, y in enumerate(self.ypoints)
                                 ], dtype=np.float32)
        else:
            verts = np.array([
                                 [
                                     x, y, 1 * self.tmp.noise2d(x=n / 5 + self.offset, y=m / 5 + self.offset) / 10
                                 ] for n, x in enumerate(self.xpoints) for m, y in enumerate(self.ypoints)
                                 ], dtype=np.float32)
        ...
        if(audint != 0):
            self.offset -= 0.2 * audint
        else:
            self.offset -= 0.1
        # self.offset -= 0.18(audio_int()/audio_int())
